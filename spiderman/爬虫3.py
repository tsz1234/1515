...
'朝向', '装修', '楼层','年份', '结构', '小区', '区域', '关注量', '发布时间'])
    ...
    def search_product():
            brower.get(f'https://cs.lianjia.com/ershoufang/rs{key}/')
    def parse_items():
        try:
    ...
    def extract_data(item):
        ...
        return {
            ...
            '楼层': house_info[4] if len(house_info) > 4 else '',
            '年份': year ,
            '结构':  structure,
            '小区': position_info[0],
    ...
    def handle_pagination():
        ...
        while current_page <= max_page:
            ...
            url = f"https://cs.lianjia.com/ershoufang/pg{current_page}/"
    ...
    def parse_items():
        try:
            items = brower.find_elements(By.CSS_SELECTOR, 'li[class*="clear"]')
            for item in items:
                # 数据提取逻辑保持不变
                data = extract_data(item)
                save_to_csv(data)
